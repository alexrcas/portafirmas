...
@ApplicationScoped
public class FirmarDocumentoSustitutoFacadeImpl implements FirmarDocumentoSustitutoFacade {
    ...
    @Override
    public void execute(FirmarDocumentoSustitutoCommand command) {
        TareaFirma tareaFirma = tareaFirmaDao.findById(command.getIdTarea());
        tareaService.firmarTareaSustituto(tareaFirma, command.getFirma());
    }

    @Override
    public UsuarioDto getUsuarioOriginal(Long idTarea) {
        TareaFirma tareaFirma = tareaFirmaDao.findById(idTarea);
        Usuario usuario = tareaFirma.getUsuario();
        return UsuarioDtoFactory.newDto(usuario);
    }
